...
public class MixedTestPaper extends AbstractTestPaper {
    private int score;
    ...
    public MixedTestPaper(int formulasQuantity) throws IOException {
        String option = null;
    ...
    private void getExerciseOfBaseFormula(int formulasQuantity) {
        isInputLegal = true;
        try (ObjectInputStream readAdditionBaseStream = new ObjectInputStream(new FileInputStream("C:/JavaFile/AdditionBase.txt"));
             ObjectInputStream readSubtractBaseStream = new ObjectInputStream(new FileInputStream("C:/JavaFile/SubtractBase.txt"))) {
            for (int i = 0; i < OperationBase.TOTAL_OPERATIONS_IN_BASE; i++) {
                AbstractBinaryOperation additionOperation = (AbstractBinaryOperation) readAdditionBaseStream.readObject();
                tempOfBase.add(additionOperation);
            }
            for (int i = 0; i < OperationBase.TOTAL_OPERATIONS_IN_BASE; i++) {
                AbstractBinaryOperation subtractOperation = (AbstractBinaryOperation) readSubtractBaseStream.readObject();
                tempOfBase.add(subtractOperation);
            }
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        } catch (IOException e) {
        ...
        for (int count = 0; count < formulasQuantity; count++) {
            ...
            formulasList.add(tempOfBase.get(randomIndex));
        }
    }
    ...
    private void getExerciseOfRandomFormula(int formulasQuantity) {
        ...
        for (int i = 0; i < formulasQuantity; i++) {
            ...
            if (random == 0)
                formulasList.add(new SubtractOperation());
            else
                formulasList.add(new AdditionOperation());
        }
    }
